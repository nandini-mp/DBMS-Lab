        ...
        if (op == NE || op == LT || op == LE) { /* op is one of NE, LT, LE */
            ...
            */
            ...
            if (found) {/* such an entry is found*/
                ...
                if (intHead.numEntries > 0) {
                    internalBlk.getEntry(&intEntry, intHead.numEntries - 1);
                    block = intEntry.rChild;
                } else {
                    // empty internal node — safe exit/handle as appropriate
                    return RecId{-1,-1};
                }
            }
        }
...
int BPlusTree::bPlusDestroy(int rootBlockNum) {
    ...
    if (type == IND_LEAF) {
        ...
        for (int i=0;i<head.numEntries;i++)
        {
            ...
            ret = BPlusTree::bPlusDestroy(internalEntry.rChild);
            if (ret!=SUCCESS) return ret;
        }
...
int BPlusTree::bPlusInsert(int relId, char attrName[ATTR_SIZE], Attribute attrVal, RecId recId) {
    ...
    if (insertIntoLeaf(relId,attrName,leafBlkNum,entry) == E_DISKFULL/*insertIntoLeaf() returns E_DISKFULL */) {
...
int BPlusTree::findLeafToInsert(int rootBlock, Attribute attrVal, int attrType) {
    ...
    while (StaticBuffer::getStaticBlockType(blockNum)!=IND_LEAF/*block is not of type IND_LEAF */) {  // use StaticBuffer::getStaticBlockType()
        ...
        if (i==head.numEntries/*no such entry is found*/) {
...
int BPlusTree::insertIntoInternal(int relId, char attrName[ATTR_SIZE], int intBlockNum, InternalEntry intEntry) {
    ...
    */
    ...
    for (int i=0;i<blockHeader.numEntries;i++)
    {
        ...
        else //find first existing key greater than new key, so insert new intEntry here
        {
            internalEntries[i]=intEntry;
            insertedIndex = i;
            for (i++;i<=blockHeader.numEntries;i++)
            {
                intBlk.getEntry(&internalEntry,i-1);
                internalEntries[i] = internalEntry;
            }
            break;
        }
    }
    if (insertedIndex == -1) //new key goes at end of array for asc order
    {
    ...
    if (blockHeader.pblock!=-1/* the current block was not the root */) {  // (check pblock in header)
        ...
        // insertIntoInternal(relId, attrName, parent of current block, new internal entry)
        return insertIntoInternal(relId,attrName,blockHeader.pblock,internEntry);
...
int BPlusTree::splitInternal(int intBlockNum, InternalEntry internalEntries[]) {
    ...
    */
    ...
    for (int i=0;i<MIDDLE_INDEX_INTERNAL;i++/* each child block of the new right block */) {
        ...
        BlockBuffer block(internalEntries[i+51].rChild);
